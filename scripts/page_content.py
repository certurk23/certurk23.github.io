...
where  BUY  &lt;=&gt;  confluence_score &gt;= {THR}
  ...
  <ul>
    ...
    under this methodology. Backfilling it would be inventing data. What the
    first twenty observations can and cannot say, and the test pre-registered
    for when the series is long enough, are in
    <a href="/learn/signal-breadth-twenty-days.html">Signal Breadth: what twenty
    observations can and cannot say</a>.</li>
...
BREADTH_LEARN_BODY = f"""
  ...
  <h2>What the first twenty observations showed</h2>
  <p>Signal Breadth started publishing on 17 August 2026 and is not backfilled.
  Over its first twenty sessions (to 14 September 2026) it ranged from 42.5%
  to 15.6%, with a median of 29.1%; behind those readings were 28 to 76 BUY
  names out of 179 scored. Two properties matter for reading it. The series is
  persistent: its one-day autocorrelation is 0.70, about three standard errors
  from white noise, because the underlying signals include slow-moving trend
  conditions. And it is almost the same information as the median score
  (correlation 0.84 across days), since both summarise one scan. A run of
  eleven down days against five up days over four weeks is therefore what one
  drawn-out decline looks like, not evidence of a regime. The full analysis,
  including how many observations a predictive test would need, is in
  <a href="/learn/signal-breadth-twenty-days.html">Signal Breadth: what twenty
  observations can and cannot say</a>.</p>

  <h2>Compute it yourself</h2>
  <p>The published JSON carries everything needed to recompute the figure, so
  it can be checked rather than trusted:</p>
  <div class="qm-formula">import json, urllib.request<br>
d = json.load(urllib.request.urlopen('https://quantmedia.io/data/signal_breadth.json'))<br>
print(d['market_date'], d['buy_signals'], '/', d['scored_stocks'], '=',<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;round(100 * d['buy_signals'] / d['scored_stocks'], 1), '%  (published:', d['breadth_pct'], ')')</div>
  <p>The two numbers must agree to one decimal; the pipeline's validator
  refuses to publish a day where they do not. Comparing the result with the
  quoted <code>threshold</code> and <code>signal_count</code> fields also shows
  whether the methodology version has changed since the figure was cited.</p>
